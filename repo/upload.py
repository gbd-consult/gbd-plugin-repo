...
from repo.helpers import md5, readline_generator, newer_version


def plugin_upload(user: User, package: io.BytesIO):
    existing_plugin = Plugin.query.filter_by(
        md5_sum=md5(package)).first()
    if existing_plugin:
    ...
    metadata_files = list(filter(
        lambda x: x.endswith('metadata.txt'), zip_file.namelist()))
    ...
    if Path(metadata_file).parent.name:
        ...
        package_name = (
                f"{name.lower().replace(' ', '_').replace('-', '_')}.zip"
            )
    ...
    old_plugin = Plugin.query.filter_by(
        file_name=package_name).first()
    if old_plugin and \
            not newer_version(version, old_plugin.version):
        return (
            False,
            f"There already exists a more recent verion of {package_name}"
        )
